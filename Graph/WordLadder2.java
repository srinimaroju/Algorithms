...
public class WordLadder2 {
	...
	public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {
		...
		//System.out.println("Calculating path between " + s + " and " + d + " at level: " + level);

		Queue<PQNode> next = new LinkedList<PQNode>();

		PQNode begin = new PQNode(s, null);
		wordSet.remove(s);
		
		next.addAll(this.getNeighbours(begin));
		//System.out.println("Next is :" + next);


		while(!next.isEmpty()) {
			PQNode nextNode = next.poll();
			//System.out.println("Processing word: " + nextNode.word);
			if(nextNode.level > maxlevel) {
				break;
			}
			if(nextNode.word.equals(d)) {
				result.add(this.getPath(nextNode));
				maxlevel = Math.min(maxlevel, nextNode.level);
			}
			wordSet.remove(nextNode.word);

			next.addAll(this.getNeighbours(nextNode));

		}
		return result;
	}

	public List<String> getPath(PQNode node) {
	...
	public List<PQNode> getNeighbours(PQNode node) {
		String s =  node.word;
		List<PQNode> result = new ArrayList<PQNode>();
		...
		for(int i=0;i<s.length();i++) {
			...
			for(char c='a';c<='z';c++) {
				if(c!=word[i]) {
					word[i]=c;
					String neighbour = String.valueOf(word);
					if(wordSet.contains(neighbour)) {
							result.add(new PQNode(neighbour, node));
	...
	class PQNode {
		...
		PQNode parent;
		int level;
		public PQNode(String word, PQNode parent) {
			this.word = word;
			this.parent = parent;
			if(parent!=null) {
